...
module.exports = class Router {
  ...
  onHistoryChange({location, action}) {
    const {isReloadNextHistoryChange} = this;
    ...
    this.promise = this.broadcastStartEvent({
      ...
      .then(() => utils.fetchResolveData(nextRoute, nextParams, reusableResolveData, this.history))
      .then(resolveData => {
        const props = {
  ...
  start() {
    ...
    this.promise = this.broadcastStartEvent({
      ...
      .then(() => utils.fetchResolveData(currentRoute, currentParams, {}, this.history))
      .then(resolveData => {
        const props = {
          ...utils.flattenResolveData(resolveData),
          key: Math.random().toString(36),
  ...
  reload() {
    ...
    this.promise = this.broadcastStartEvent({
      ...
      .then(() => utils.fetchResolveData(route, params, {}, this.history))
      .then(resolveData => {
        const props = {
          ...utils.flattenResolveData(resolveData),
          key: Math.random().toString(36),
